...
export type ToolName = 'comment' | 'edit' | 'generate' | 'formatBlock' | 'insertBlock';
...
export type TFormatBlockPart = {
  formatBlock: { align?: 'left' | 'center' | 'right' | 'justify'; indent?: 'increase' | 'decrease' } | null;
  status: 'streaming' | 'finished';
};

export type TInsertBlockPart = {
  insertBlock: { at: 'cursor' | 'afterSelection'; blockType: string; content?: string; icon?: string } | null;
  status: 'streaming' | 'finished';
};
...
export type MessageDataPart = {
  ...
  formatBlock?: TFormatBlockPart;
  insertBlock?: TInsertBlockPart;
};
...
const createTableCellChunks = (editor: PlateEditor) => {
  ...
  if (selectedCells.length > 0) {
    ...
    cellIds = (cells as Array<[{ id?: string }, unknown]>)
      .map(([node]) => node.id)
