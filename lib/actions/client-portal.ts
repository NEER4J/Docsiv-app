...
export async function requestClientPortalMagicLink(clientId: string, rawEmail: string): Promise<{ error?: string }> {
  ...
  const next = `/client/${clientId}`;
  const emailRedirectTo = `${origin}/auth/callback?next=${encodeURIComponent(next)}`;

  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  // Preferred path: generate magic link with Supabase Admin and send via Resend.
  if (supabaseUrl && serviceRoleKey && process.env.RESEND_API_KEY) {
    ...
    if (!actionLink) {
      return { error: "Could not generate portal magic link." };
    }
  ...
  // Fallback: Supabase OTP mailer (if Resend/service-role isn't configured yet).
...
export async function activateClientPortalMembership(clientId: string): Promise<{ allowed: boolean; error?: string }> {
  const supabase = await createClient();
  const hostWorkspace = await resolveWorkspaceByHost();
  if (!hostWorkspace || !hostWorkspace.hide_docsiv_branding) {
